...
router.post("/StrikeStages", async (req, res) => {
    try {
        ...
        if (!CheckIfArray(stages, res) || stages.length == 0) return SetJSONResponse(res, definitionErrors.stagesUndefined);
...
router.post("/PickStage", async (req, res) => {
    try {
        ...
        if (!CheckUserDefined(req)) return SetJSONResponse(res, userErrors.notLoggedIn);
        if (typeof(stage) !== 'number') return SetJSONResponse(res, definitionErrors.stageUndefined);
...
router.post("/WinGame", async (req, res) => {
    try {
        ...
        if (!CheckUserDefined(req)) return SetJSONResponse(res, userErrors.notLoggedIn);
        if (typeof(winnerId) !== 'string') return SetJSONResponse(res, definitionErrors.winnerUndefined);
...
router.post("/SendChatMessage", async (req, res) => {
    try {
        ...
        if (!CheckUserDefined(req)) return SetJSONResponse(res, userErrors.notLoggedIn);
        if (await CheckUserBanned(userId)) return SetJSONResponse(res, userErrors.banned);
...
router.post("/LoadChatMessages", async (req, res) => {
    try {
        ...
        if (typeof(matchId) !== 'string') return SetJSONResponse(res, definitionErrors.matchUndefined);
        if (typeof(loadedMessagesAmount) !== 'number') return SetJSONResponse(res, definitionErrors.chatMessageUndefined);
        if (loadedMessagesAmount < 0) return SetJSONResponse(res, definitionErrors.chatMessageUndefined);
        ...
        if (!match){
            ...
            if (!matchData) return SetJSONResponse(res, nullErrors.noMatch);
        ...
        if (!players[0] == userId && !players[1] == userId){
            ...
            if (userRole != userRoles.mod || match.privateBattle){
                return SetJSONResponse(res, userErrors.noAccess);
            }
        }
...
router.post("/GetMatchInfo", async (req, res) => {
    try {
        ...
        if (!CheckIfPlayerIsId(players[0], userId) && !CheckIfPlayerIsId(players[1], userId)){
            //mods cant see PBs
            if (userRole != userRoles.mod || match.privateBattle){
                if (matchHidden) return SetJSONResponse(res, userErrors.noAccess);
                match.chat = [];
            }
        }
